...
#MOVIE-3 시청중인 목록에 요청한 영화가 있으면 이어보기
query_text = input('영화 이름 입력: ')

for k in page:
    page_URL_movie = 'https://www.wavve.com/my/use_list_movie_history?page=' + str(k)
    driver.get(url = page_URL_movie)
    try:
        movie_title = driver.find_element_by_partial_link_text(query_text)
        if movie_title != 0:
            print(movie_title)
            movie_title.click()
            watching_movie_val = 1
            break;
    except:
        watching_movie_val = 0

##SEARCH-MOVIE
#1 시청중인 영화 목록에 없을 경우 검색
if watching_movie_val == 0:
